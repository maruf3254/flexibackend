...
export const BookCar = async (req, res, next) => {
  try {
    ...
    if (!vehicleId || !pickupLocation || !pickupTime || !destination || !email) {
      return next(errorHandler(400, "All required fields must be provided"));
    }

    // 🔥 Convert pickupTime properly
    const pickupDate = new Date(pickupTime);

    if (isNaN(pickupDate.getTime())) {
      return next(errorHandler(400, "Invalid pickup time"));
    }
    ...
    // ✅ Now use Date, NOT raw pickupTime
    ...
    if (existingBooking) {
      return next(errorHandler(409, "Vehicle already booked for this time"));
    }
    ...
    console.error(error);
    next(errorHandler(500, "Failed to book vehicle"));
  }
};

export const getAllBookings = async (req, res, next) => {
  try {
    // Fetch all bookings, populate user and vehicle info
    const bookings = await Booking.find()
      .populate("userId", "username email phoneNumber")   // only select fields you need
      .populate("vehicleId", "model registeration_number Seats description imagePath") // vehicle info
      .sort({ pickupTime: -1 }); // latest bookings first
