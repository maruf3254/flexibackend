...
/**
 * Get all vehicles or filtered vehicles based on search criteria
 * @route POST /api/user/searchCar
 * @access Public
 */
export const getVehicles = async (req, res, next) => {
  try {
    const { division, district, upazila, zone } = req.body || {};

    console.log(division, district, upazila, zone)
    ...
    // Only return vehicles that are not deleted (if you add soft delete later)
    // filter.isDeleted = false; // optional
    ...
    res.status(200).json({
      success: true,
      message: `${vehicles.length} vehicle(s) found`,
      data: vehicles,
    });
  } catch (error) {
    console.error("Error fetching vehicles:", error);
    next(errorHandler(500, "Something went wrong while fetching vehicles"));
  }
};

// GET all vehicles for users
export const listAllVehicles = async (req, res, next) => {
  try {
    const vehicles = await vehicle.find(); // Only non-deleted vehicles
    ...
    res.status(200).json({
      success: true,
      data: vehicles,
      message: `${vehicles.length} vehicles found`,
    });
  } catch (error) {
    console.error(error);
    next(errorHandler(500, "Something went wrong while fetching vehicles"));
